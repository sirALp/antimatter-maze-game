...
int displayMenu(){
	...
	while( output == 0 || output == 1){
		if (output == 1){
			...
			if( !printMapsFolder(fileName) ) printf("\n\t\t! Couldn't read the folder/files !");
			strcat(fileName,".txt");
			field = readGameField(fileName);
			if(field != NULL){ strcpy(settings.fileName,fileName); }
			output = printLobby();
			free(fileName);
		}
		else output = printLobby();
	}
	if(output == -1){
		return 	settings.autoMod ? 
				_A_GAME_(field,settings.difficulty) :
				_M_GAME_(field,settings.difficulty) ;
	}
	else return -1; // back to main
...
int _A_GAME_(FIELD* field,_Bool isHard){
	...
	while ( 1 ){
		...
		if (nextSquare == END){
			...
			displayGameField(field,moveCounter);
			printf("\n\t\t$$ AUTO MOD $$  --> 'Q' to quit\n\n\t\tCollected Items: ");
			printf("\n\nDIRECTIONS - head->%s",	player->head == 0 ? "UP" 	: 
												player->head == 1 ? "RIGHT"	:
												player->head == 2 ? "DOWN" 	: "LEFT");
			Sleep(250);
			for(i=0;i<itemCount;i++) { printf("%c ",items[i]); }
			
			if ( (nextSquare = move(field,1,directions[player->leftHandSide],items,&itemCount)) >= 1){ // moved to left-hand-side
				player->leftHandSide--; player->head--; 
			}
			else if( (nextSquare = move(field,1,directions[player->head],items,&itemCount)) >= 1); // kept going straight
			else {	player->leftHandSide++; player->head++; }		// couldn't do either so turned right
			moveCounter--;
		}
	}		
}
...
int _M_GAME_(FIELD* field,_Bool isHard){
	...
	copyField = field;
	int moveCounter = isHard ? 	field->fieldSize[0] * field->fieldSize[1] / 6 + 5 :
								field->fieldSize[0] * field->fieldSize[1] / 4 + 5 ;
	while(1){
		system("cls");
		displayGameField(field,moveCounter);
		...
		if ( nextSquare >= 1 && nextSquare <= ANTIPROTON ) moveCounter--;
		
		if(nextSquare == END){
			int p = calcPoints(items,itemCount);
			if (isHard && p>0) p = 1.5 * p; 
...
int move(FIELD* field,_Bool isAuto,int dir,char* bag,int* itemCount){
	...
	if ( !isAuto ) _Move = getch();
	else _Move = dir;
	
	switch (_Move){
		case RIGHT_ARROW:
			if(field->curLocation[1] == field->fieldSize[1] -1) return 0;
			else canMove = moveable(field->gameField[field->curLocation[0]][field->curLocation[1]+1]);
			if (isAuto == 1 && canMove == BLACKHOLE) canMove = 0; // for auto mod 
			if(canMove >= 1) 
				field->curLocation[1]++;
			return canMove;
		case LEFT_ARROW:
			if(field->curLocation[1] == 0) canMove = 0;
			else canMove = moveable(field->gameField[field->curLocation[0]][field->curLocation[1]-1]);
			if (isAuto && canMove == BLACKHOLE) canMove = 0; // for auto mod 
			if(canMove >= 1) 
				field->curLocation[1]--;
			return canMove;
		case UP_ARROW:
			if(field->curLocation[0] == 0) canMove = 0;
			else canMove = moveable(field->gameField[field->curLocation[0]-1][field->curLocation[1]]);
			if (isAuto && canMove == BLACKHOLE) canMove = 0; // for auto mod 
			if(canMove >= 1) 
				field->curLocation[0]--;
			return canMove;
		case DOWN_ARROW:
			if(field->curLocation[0] == field->fieldSize[0]-1) canMove = 0;
			else canMove = moveable(field->gameField[field->curLocation[0]+1][field->curLocation[1]]);
			if (isAuto && canMove == BLACKHOLE) canMove = 0; // for auto mod 
			if(canMove >= 1)
				field->curLocation[0]++;
			return canMove;
		case Q_KEY:	return Q_KEY;
		case R_KEY: return R_KEY;
	}
	...
	int moveable(char des){
		...
		if(des == 'K' || des == 'k') 	return -1;
		if(des == 'C' || des == 'c')	return  2;
...
int calcPoints(char bag[],int itemCount){
	auto void printBag(char* b,int i,int c1,int c2){
		...
		printf("\33[2K\r\t\tBag : ");
		for(j=0;j<i;j++){
		 	if(j == c1)	printf("[%c] ",bag[j]);
			else if(j == c2) printf("[%c] ",bag[j]); 
			else printf("%c ",bag[j]);
		}
	}
	...
	while(index < itemCount){
		...
		while ( temp < itemCount){
			...
			if ( diff == 32){
				...
				for(i=temp; i<=itemCount-2;i++)	bag[i] = bag[i+1];	// righthand-side item removed
				itemCount--;	
				for(i=index; i<=itemCount-2;i++)	bag[i] = bag[i+1];	// lefthand-side item removed
				itemCount--;	index--;
				Sleep(3000);
	...
	for (index = 0; index<itemCount ;index++)
		if (bag[index] == ANTIELECTRON) antiElec++;
	for (index = 0; index<itemCount ;index++)
		if (bag[index] == ANTIPROTON) antiProt++;
